...
def drop_na_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_vars_with_na = [
        ...
        + config.model_config.numerical_vars_with_na #because there will be imputation for missing values in these
...
def validate_inputs(*, input_data: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[dict]]:
    ...
    try:
        ...
        MultipleHouseDataInputs(
            inouts= validated_data.replace({np.nan:None}).to_dict(orient="records")
        )
